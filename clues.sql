...
-- Write SQL query here
SELECT countrycode FROM countrylanguages GROUP BY countrycode HAVING COUNT(countrycode) = 1 AND MAX(language) = 'Italian';
...
-- Write SQL query here
SELECT cities.countrycode FROM cities JOIN countries ON cities.countrycode = countries.code WHERE cities.name ~ 'Serra' AND countries.region = 'South America';

-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towardsq
